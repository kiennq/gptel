...
(defun gptel--gh-get-registered-usernames ()
  (thread-last gptel--known-backends
    (mapcar #'cdr)
    (seq-filter #'gptel--gh-p)
...
instead of attempting to open a browser automatically."
  (interactive (list (let ((known-usernames
                            (mapcar (lambda (name)
                                      (if (string= name "")
                                          `(,gptel--gh-default-username-placeholder . ,name)
                                        `(,name . ,name)))
                                    (gptel--gh-get-registered-usernames))))
                       (cond
                        ...
                        (t
                         (assoc
                          (completing-read "Choose GitHub username: " known-usernames
                                           nil t)
                          known-usernames #'string=))))))
  (message "Logging in using '%s'" (if (consp github-username)
                                       (car github-username)
                                     github-username))
  (setq github-username (if (consp github-username)
                            (cdr github-username)
                          github-username))
